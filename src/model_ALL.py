...
def build_siamese_seq_factor_bin(
    ...
    zB = embed_triplet(B_seq, B_factor, B_bin)
    zC = embed_triplet(C_seq, C_factor, C_bin)
...
if __name__ == "__main__":
    # ---  保存ディレクトリ名の設定 ---
    # ディレクトリ名
    EXPERIMENT_NAME = "All_model_ver3_baseline" 
    ...
    indices_train, indices_test, X_train_A_seq, X_test_A_seq, X_train_A_factor, X_test_A_factor, X_train_A_bin, X_test_A_bin, X_train_B_seq, X_test_B_seq, X_train_B_factor, X_test_B_factor, X_train_B_bin, X_test_B_bin, X_train_C_seq, X_test_C_seq, X_train_C_factor, X_test_C_factor, X_train_C_bin, X_test_C_bin, \
    ...
    scores = (dAB_test - dAC_test).flatten()
...
# ★ 1. ロードするデータセットファイルの指定 ★
    ...
    rX_A_bin = np.array([transform_sequence_diff(seq) for seq in rX_A_raw], dtype=np.float64).reshape(-1, MAX_SEQUENCE_LENGTH-1)
    rX_B_bin = np.array([transform_sequence_diff(seq) for seq in rX_B_raw], dtype=np.float64).reshape(-1, MAX_SEQUENCE_LENGTH-1)
    rX_C_bin = np.array([transform_sequence_diff(seq) for seq in rX_C_raw], dtype=np.float64).reshape(-1, MAX_SEQUENCE_LENGTH-1)
    ...
    rX_A_bin = np.array([sequence_to_bins(seq, edges) for seq in rX_A_bin], dtype=np.int32)
    rX_B_bin = np.array([sequence_to_bins(seq, edges) for seq in rX_B_bin], dtype=np.int32)
    rX_C_bin = np.array([sequence_to_bins(seq, edges) for seq in rX_C_bin], dtype=np.int32)

    print("--- Check rX Types and Shapes ---")
    # 全ての rX_* 変数について確認
    for name, var in {
        "rX_A_seq": rX_A_seq, "rX_A_factor": rX_A_factor, "rX_A_bin": rX_A_bin,
        "rX_B_seq": rX_B_seq, "rX_B_factor": rX_B_factor, "rX_B_bin": rX_B_bin,
        "rX_C_seq": rX_C_seq, "rX_C_factor": rX_C_factor, "rX_C_bin": rX_C_bin,
    ...
    rdAB_test, rdAC_test = model.predict({
        "A_seq": rX_A_seq, "A_factor": rX_A_factor, "A_bin": rX_A_bin,
        "B_seq": rX_B_seq, "B_factor": rX_B_factor, "B_bin": rX_B_bin,
        "C_seq": rX_C_seq, "C_factor": rX_C_factor, "C_bin": rX_C_bin,
    ...
    # --- サマリーレポートの作成 ---
    with open(os.path.join(save_dir, "randam_seqA_summary.txt"), "w") as f:
        f.write(f"Experiment Name: {EXPERIMENT_NAME}\n")
        f.write(f"Date: {datetime.now()}\n")
        f.write("-" * 30 + "\n")
        f.write(f"ACC: {accuracy_score(ry_true_pm1, ry_pred_pm1):.4f}\n")
        f.write(f"P  : {precision_score(ry_true_pm1, ry_pred_pm1):.4f}\n")
        f.write(f"R  : {recall_score(ry_true_pm1, ry_pred_pm1):.4f}\n")
        f.write(f"F1 : {f1_score(ry_true_pm1, ry_pred_pm1):.4f}\n")
        f.write(f"AUC: {roc_auc_score(ry_true_pm1, rscores):.4f}\n")
