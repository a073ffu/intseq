...
def build_siamese_seq_factor(
    ...
    model = keras.Model(inputs=[A_seq, A_factor], outputs=output)
...
if __name__ == "__main__":
    ...
    # ---  保存ディレクトリ名の設定 ---
    # ディレクトリ名
    EXPERIMENT_NAME = "ST2_model_DistanceThreshold_single_encoder_baseline" 
    ...
    indices_train, indices_test, X_train_A_seq, X_test_A_seq, X_train_A_factor, X_test_A_factor, \
    y_train, y_test = train_test_split(
        indices,
        X_B_seq, X_B_factor, y_labels,
    ...
    # 訓練データを検証データと分割
    X_train_A_seq, X_val_A_seq, X_train_A_factor, X_val_A_factor, \
    y_train, y_val = train_test_split(
        X_train_A_seq, X_train_A_factor, y_train,
        test_size=1 - TRAIN_TEST_SPLIT_RATIO, 
        random_state=42,
    ...
    for i in range(len(metrics)):
        ...
        plt.legend()  # ラベルの表示
            # 画像ファイルとして保存
        PLOT_FILENAME = 'learning_curve_' + metric + '_factor.png'
        plt.savefig(PLOT_FILENAME)
        plt.close() # メモリ解放
        print(f"\nLearning curve (Loss & Accuracy) saved to {PLOT_FILENAME}")
    '''
